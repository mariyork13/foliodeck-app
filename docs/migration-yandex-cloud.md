...
**Актуальный план — ВЫПОЛНЕНО (2026-09-10):**

- **Приложение осталось на Vercel** — не трогали.
- **База данных → Timeweb Cloud, PostgreSQL 17, Москва** (744₽/мес).
  ✅ Кластер создан, данные перенесены (`scripts/migrate-to-ru.mjs`,
  ~3200 строк, счётчики сходятся), запросы приложения проверены.
- **Картинки (51 файл) → Timeweb Object Storage, бакет `foliodeck-media`,
  СПб** (79₽/мес). ✅ Перенесены (`scripts/migrate-images-to-ru.mjs`),
  ссылки в БД переписаны, отдаются с `s3.twcstorage.ru`.
- **Vercel:** переменные `DATABASE_URL_RU` + `S3_*` + `MEDIA_PUBLIC_BASE`
  добавлены в Production и Preview; ветка `migrate/russia` влита в `main`
  (коммит cb0de65 + фикс 3e08ffc), задеплоено.
- ✅ Проверено на `foliodeck-app.vercel.app`: главная 200, обложки грузятся
  с Timeweb S3, ссылок на Vercel Blob не осталось, `/admin` и `/designer`
  на публичном хосте → 404, внутренний хост `dizain-tashchit-baza.vercel.app`
  работает.
- Разделы 2–6 ниже (VM, Docker, Caddy, systemd, ALB) **не актуальны** —
  оставлены для истории.

**Осталось:**

1. ✅ Загрузка картинки через админку работает (файл ушёл в бакет `foliodeck-media`).
2. Юрист: privacy policy теперь соответствует действительности («база в РФ»);
   уведомление РКН (оператор ПДн + трансграничная передача — приложение на
   Vercel за рубежом читает РФ-базу).
3. Домен `foliodeck.pro` — отдельная задача, когда пойдём в публичность.
4. ✅ Rate-limit / honeypot на форме подачи (honeypot-поле `company` +
   лимит 3/час на IP и 20/час суммарно, считается по `portfolio_submissions`).
5. Через ~2 недели стабильной работы: удалить проект Neon и Vercel Blob store;
   вынести секреты из `.env.local` (iCloud).
