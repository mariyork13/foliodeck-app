...
| `NEXT_PUBLIC_YM_COUNTER_ID` | Номер счётчика Яндекс.Метрики (публичная переменная). Не задан → аналитика не подключается, баннер показывает только «необходимые cookie». | `123456789` |
...
- **Аналитика — Яндекс.Метрика, только по согласию** (`src/lib/consent-context.tsx`,
  `src/components/yandex-metrica.tsx`, `src/components/cookie-banner.tsx`):
  - Куки-баннер при первом заходе: «Принять» / «Только необходимые». Выбор в
    `localStorage` (`foliodeck-cookie-consent` = `all` / `necessary`).
  - Метрика (`NEXT_PUBLIC_YM_COUNTER_ID`) грузится **только** при согласии `all`
    и только если ID задан. **Webvisor выключен** (форма подачи содержит ПДн),
    clickmap/scrollmap включены. SPA-переходы отслеживаются вручную по
    `usePathname` (`ym(id, 'hit', ...)`, `defer: true`).
  - Метрика подключена только на публичном сайте (в группе `(site)`), не в
    админке.
  - Тексты в политике конфиденциальности (§2.3 и §10, RU+EN) обновлены под
    Метрику — **требуют финальной проверки юристом**.
...
- **DNS без Cloudflare-проксирования** — принципиально: оранжевое облако
  ...
  доступ из РФ.
- **Аналитика** — Яндекс.Метрика, грузится только после согласия в куки-баннере,
  без Webvisor. Данные у Яндекса в РФ (совместимо с локализацией). Любая
  замена/добавление аналитики → обновление политики конфиденциальности +
  проверка юристом.
